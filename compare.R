...
bijcheck <- function(group_ids_1, group_ids_2, min_group_size) {
  ...
  valid_groups_1 <- as.integer(names(count_table_1))[
...
drop_singletons <- function(group_ids, label = "groups") {
  ...
  if (length(singletons) > 0L) {
    message(sprintf(
      "  %s: reassigning %d singleton ids to -1",
      label,
      length(singletons)
    ))
...
if (nrow(joined) == 0L) {
  stop(
    "No galaxies matched between the generated and mock catalogues. ",
...
cat(sprintf(
  "  E_FoF  = %.4f   (%d / %d generated groups bijective)\n",
  E_FoF,
  fof_vs_mock$e_num,
  fof_vs_mock$e_den
))
cat(sprintf(
  "  E_mock = %.4f   (%d / %d mock groups bijective)\n",
  E_mock,
  mock_vs_fof$e_num,
  mock_vs_fof$e_den
))
